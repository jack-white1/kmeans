...
__global__ void assign_labels(float* centroids, float* particles, int32_t* output, int32_t dim, int32_t nParticles, int32_t nCentroids) {
    ...
    for (long int tileIndex = 0; tileIndex < tilesPerBlock; tileIndex++){
		...
		for (int i = 0; i < 8; i++){
			temp[i] = startingPointer4[i*32 + threadIdx.x];
			particlesTile[threadIdx.x * 4 + 128 * i] = __float2half(temp[i].x);
			particlesTile[threadIdx.x * 4 + 128 * i + 1] = __float2half(temp[i].y);
			particlesTile[threadIdx.x * 4 + 128 * i + 2] = __float2half(temp[i].z);
			particlesTile[threadIdx.x * 4 + 128 * i + 3] = __float2half(temp[i].w);
		}
