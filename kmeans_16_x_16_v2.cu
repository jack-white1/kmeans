...
__global__ void assign_labels(float* centroids, float* particles, int32_t* output, int32_t dim, int32_t nParticles, int32_t nCentroids) {
    ...
    for (long int tileIndex = 0; tileIndex < M / NBLOCKS / K; tileIndex++){
		...
		float4 *float4Pointer = reinterpret_cast<float4*>(&(particles[particlesOffset]));
#ifndef DOUBLE_BUFFER
		float4 temp1, temp2;

		temp1 = float4Pointer[threadIdx.x];
		temp2 = float4Pointer[threadIdx.x + 32];
		...
		if (tileIndex == 0) {
            ...
            float4 temp0 = float4Pointer[threadIdx.x];
            float4 temp1 = float4Pointer[threadIdx.x + 32];

            particlesTile[4*threadIdx.x] = __float2half(temp0.x);
            particlesTile[4*threadIdx.x+1] = __float2half(temp0.y);
			particlesTile[4*threadIdx.x+2] = __float2half(temp0.z);
			particlesTile[4*threadIdx.x+3] = __float2half(temp0.w);
			particlesTile[4*threadIdx.x+128] = __float2half(temp1.x);
			particlesTile[4*threadIdx.x+129] = __float2half(temp1.y);
			particlesTile[4*threadIdx.x+130] = __float2half(temp1.z);
			particlesTile[4*threadIdx.x+131] = __float2half(temp1.w);

            // Start asynchronous copy for the next tile
            __pipeline_memcpy_async(&(nextParticlesTile[threadIdx.x]), &(float4Pointer[threadIdx.x]), 16, 0);
			__pipeline_memcpy_async(&(nextParticlesTile[threadIdx.x + 32]), &(float4Pointer[threadIdx.x + 32]), 16, 0);
            __pipeline_commit();
			...
			particlesTile[4*threadIdx.x] = __float2half(nextParticlesTile[threadIdx.x].x);
			particlesTile[4*threadIdx.x+1] = __float2half(nextParticlesTile[threadIdx.x].y);
			particlesTile[4*threadIdx.x+2] = __float2half(nextParticlesTile[threadIdx.x].z);
			particlesTile[4*threadIdx.x+3] = __float2half(nextParticlesTile[threadIdx.x].w);
			particlesTile[4*threadIdx.x+128] = __float2half(nextParticlesTile[threadIdx.x+32].x);
			particlesTile[4*threadIdx.x+129] = __float2half(nextParticlesTile[threadIdx.x+32].y);
			particlesTile[4*threadIdx.x+130] = __float2half(nextParticlesTile[threadIdx.x+32].z);
			particlesTile[4*threadIdx.x+131] = __float2half(nextParticlesTile[threadIdx.x+32].w);
